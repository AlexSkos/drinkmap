...
export type TransKey = keyof typeof STR["en"];
...
export const STR = {
  en: {
    ...
    // история (заголовок + текст)
    historyTitle: "Valencia fountains guide",
    guideTitle: "Valencia fountains guide", // на всякий случай оставим
    historyText:
      `History of Drinking Fountains in Valencia
...
Today, Valencia has installed around 50 modern filtered drinking fountains across the city, where anyone can freely refill bottles. This initiative promotes both sustainability and accessibility.`,
  ...
  es: {
    ...
    guide: "Guía de las fuentes de Valencia",
    contact: "Contáctanos",
    ...
    send: "Enviar",
    thanks: "¡Gracias!",
    message_queued_stub: "Tu mensaje ha sido puesto en cola (simulación).",

    historyTitle: "Guía de las fuentes de Valencia",
    guideTitle: "Guía de las fuentes de Valencia",
    historyText:
      `Historia de las Fuentes de Agua Potable en Valencia
...
Hoy en día, Valencia cuenta con unas 50 fuentes potables modernas con filtrado, donde cualquiera puede rellenar su botella de forma gratuita. Una medida que fomenta la sostenibilidad y el acceso universal al agua.
    ...
    minAmount: "La donación mínima es 1 €",
    or: "o",
    typeAmount: "Escribe tu cantidad",
    continue: "Continuar",
  },
} as const;

type Lang = "en" | "es";
type LangKeys = keyof typeof STR["en"];
...
const LanguageContext = React.createContext<Ctx>({
  lang: "en",
  setLang: () => {},
  // важно: привести к типу Ctx["t"], чтобы удовлетворить generic-подпись
  t: ((k: LangKeys) => STR.en[k]) as Ctx["t"],
});


export const LanguageProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  const [lang, setLang] = React.useState<Lang>("en");
  const t = React.useCallback(<K extends LangKeys>(k: K) => STR[lang][k], [lang]);
  return (
